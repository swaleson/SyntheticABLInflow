%Based on the input file/software by fengguoFUAS (2022),
%distributed under the MIT License.

%Modified by Waleson et al. 2026 for:
%Turbulent inflow generation for atmospheric boundary layer flows (II): Comparative study of synthetic methods

%Modifications include:
%case '4096x64x64'   
%Turbulence.Field.n1: 		4096	-> 65536; % steps in x, ; % steps in x 32768  %16384 %65536
%Turbulence.Field.dx:    	8		-> 2.816;    % steps size in x
%Turbulence.Field.dy:       4.5		-> 21.25;    % steps size in y
%Turbulence.Field.dz:       4.5		-> 15.625;    % steps size in z
%Turbulence.Field.HRef:     119		-> 180;  % reference height

% The original copyright notice and MIT License are provided in LICENSE.
...
Turbulence.Field.dy          = 42.5    % steps size in y
Turbulence.Field.dz          = 31.25;    % steps size in z
Turbulence.Field.seed        = seednum;    % seed number
Turbulence.Field.HRef        = 180;  % horizontal wind speed
...
case '4096x64x64'   
Turbulence.Field.NComp       = 3;    % number of velocity component, fixed
Turbulence.Field.n1          = 65536; % steps in x 32768  %16384 %65536
Turbulence.Field.n2          = 64;   % steps in y 128
Turbulence.Field.n3          = 64;   % steps in z 128
Turbulence.Field.t_upstream  = t_upstream; % time slots
Turbulence.Field.n_upstream  = length(Turbulence.Field.t_upstream); % length of time slots, also the number of upstream yz planes
Turbulence.Field.dx          = 2.816;    % steps size in x
Turbulence.Field.dy          = 21.25;    % steps size in y
Turbulence.Field.dz          = 15.625;    % steps size in z
Turbulence.Field.seed        = seednum;    % seed number
Turbulence.Field.HRef        = 180;  % horizontal wind speed
...
Turbulence.Field.t_upstream  = t_upstream; % time slots
Turbulence.Field.n_upstream  = length(Turbulence.Field.t_upstream); % length of time slots, also the number of upstream yz planes
Turbulence.Field.dx          = 1;    % steps size in x
Turbulence.Field.dy          = 10.625;    % steps size in y
Turbulence.Field.dz          = 7.813;    % steps size in z
Turbulence.Field.seed        = seednum;    % seed number
Turbulence.Field.HRef        = 180;  % horizontal wind speed
